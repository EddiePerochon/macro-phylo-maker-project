...
.compute_crown_metrics <- function(TREE, ig) {
  ...
  if (length(cr) == 0 || is.na(cr)) {
    warning(
      sprintf(
        "Skipping genus %s: no ingroup MRCA (n=%d ingroup tips after filtering)",
        genus,
        length(ingroup_tips)
      ),
      call. = FALSE
...
.normalize_stem_mode <- function(x) {
  if (is.null(x) || is.na(x) || !nzchar(x)) return("outgroup")
  v <- tolower(trimws(as.character(x)))
  if (v %in% c("outgroup", "stem", "with_outgroup")) return("outgroup")
  if (v %in% c("crown", "crown_only", "no_outgroup", "ingroup_only")) return("crown")
...
prepare_clade_template <- function(
  ...
  orig_labels <- tr$tip.label
  ## Clean labels: extract Genus_species-looking strings only
  clean_species_label <- function(x) {
    ...
    paste(hits[2], hits[3], sep = "_")
  }

  cleaned <- vapply(tr$tip.label, clean_species_label, character(1))
  keep    <- !is.na(cleaned)

  tr <- ape::keep.tip(tr, orig_labels[keep])

  tr$tip.label_original <- tr$tip.label
  tr$tip.label <- cleaned[keep]
  ...
  ## fallback if ingroup detection collapsed
  ...
  if (!is.null(valid)) {
    keep_ig <- intersect(ingroup_tips, gsub(" ", "_", valid))
    log_section(logger, "Template: authority filter")
    log_msg(logger, "Valid ingroup species kept: ", length(keep_ig),
            "; dropped absent: ", length(setdiff(ingroup_tips, keep_ig)))
    if (!length(keep_ig))
      return(list(ok = FALSE, reason = "No ingroup retained after authority filter"))
    tr <- if (!is.na(out_tip)) ape::keep.tip(tr, c(keep_ig, out_tip))
          else ape::keep.tip(tr, keep_ig)
...
if (identical(tree_type, "chronogram")) {
  ...
  out <- list(
    ...
    original_labels = tr$tip.label_original,
...
graft_template_onto_backbone <- function(
  ...
  stem_mode <- .normalize_stem_mode(
    placement$stem_mode %||% template$stem_mode
  )
  ...
  ## ---- derive ingroup metadata from template (do NOT rely on globals) ----
  ingroup_tips <- template$ingroup_tips %||% character(0)
  genus <- template$genus %||% NA_character_
  ...
  if (identical(placement$type, "clade")) {
    ...
    ## ---- MRCA sanity check ----
    utips <- unique(ingroup_tips)

    if (length(utips) < 2) {
      warning(
        sprintf(
          "Skipping genus %s: fewer than 2 unique ingroup tips after filtering (%d)",
          genus,
          length(utips)
        ),
        call. = FALSE
      )
      return(list(ok = FALSE, reason = "Insufficient ingroup tips for MRCA"))
    }
    ...
    if (stem_mode == "crown") {
      ...
      log_row <- transform(
        template$template_log,
        target_type = "clade",
        target_label = paste(anchors, collapse = ","),
        retained_tip = NA
      )
      return(list(tree = out, log = log_row))
    }
...
graft_many_clades <- function(
  ...
  for (i in seq_along(Blist)) {
    ...
    for (nm in names(templates)) {
      ...
      pl <- placements[[nm]]
      labels <- T$tip.label
      if (!is.null(T$tip.label_original)) {
        labels <- c(labels, T$tip.label_original)
      }
      ...
      ok_target <- if (pl$type == "tip") {
        ...
        length(intersect(pl$anchors, labels)) >= 2
      }
      if (!ok_target) {
      ...
      if (!is.list(res) || is.null(res$log) || !is.data.frame(res$log)) {
